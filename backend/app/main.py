...
def current_user(x_user_id: str | None):
    if not x_user_id or not repo.users.get(x_user_id): raise HTTPException(401, "Create a profile first")
    return x_user_id


@app.get("/api/health")
async def health(): return {"ok": True, "demoMode": not bool(settings.ticketmaster_api_key)}
...
async def list_events(city: str = Query(settings.default_city), from_date: str | None = Query(None, alias="from"), to_date: str | None = Query(None, alias="to"), x_user_id: str | None = Header(None)):
    return await events.list_events(city, current_user(x_user_id), from_date, to_date)


@app.get("/api/events/calendar")
async def calendar(month: str, city: str = settings.default_city, x_user_id: str | None = Header(None)):
    return await events.calendar(city, current_user(x_user_id), month)


@app.post("/api/rsvp")
async def save_rsvp(payload: RsvpRequest, x_user_id: str | None = Header(None)):
    try: return await rsvps.save(current_user(x_user_id), payload.model_dump())
    except ValueError as exc: raise HTTPException(400, str(exc))


@app.delete("/api/rsvp/{event_id}")
async def delete_rsvp(event_id: str, x_user_id: str | None = Header(None)):
    current_user(x_user_id); return {"deleted": bool(await repo.delete_rsvp(x_user_id, event_id))}
...
async def update_reminder(event_id: str, payload: ReminderUpdate, x_user_id: str | None = Header(None)):
    user_id = current_user(x_user_id); record = await repo.get_rsvp(user_id, event_id)
...
async def dashboard(x_user_id: str | None = Header(None)): return await rsvps.dashboard(current_user(x_user_id))


@app.post("/api/share")
async def create_share(event_id: str, x_user_id: str | None = Header(None)):
    try: return await shares.create(current_user(x_user_id), event_id)
